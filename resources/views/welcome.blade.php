...
<body class="bg-slate-100 text-gray-800">
    ...
    <section id="menu" class="py-16 relative">
        ...
        <div class="container pb-8 relative">
            ...
            <div id="menu-container" class="grid grid-cols-1 md:grid-cols-3 mx-6 gap-12">
                @foreach ($menu->take(6) as $item)
                    <div class="bg-white shadow-md rounded-xl p-6 hover:scale-90 hover:transition-all hover:duration-300 opacity-0 translate-y-10 transition-all duration-700 ease-in-out fade-in">
                        <img src="{{ Storage::url($item->gambar) }}" alt="Makanan" class="w-full h-60 object-cover rounded-lg mb-4">
                        <h3 class="text-xl font-semibold">{{ $item->nama }}</h3>
                        <p class="text-gray-600">{{ $item->deskripsi }}</p>
                        <div class="flex justify-between items-center mt-4">
                            <div class="flex justify-start">
                                <p class="text-gray-700 text-lg font-bold">Rp{{ number_format($item->harga, 0, ',', '.') }}</p>
                            </div>
                            <div class="py-4 flex justify-end">
                                @foreach ($kontak as $kontakItem)
                                    <a href="https://wa.me/{{ $kontakItem->wa }}" class="bg-red-600 text-white px-4 py-2 rounded-3xl">Pesan</a>
                                @endforeach
                            </div>
                        </div>
                    </div>
            ...
            <div class="text-center mt-8">
                <button id="load-more" class="bg-red-600 text-white px-6 py-3 rounded-full hover:bg-red-700 transition">Selengkapnya</button>
            </div>
        </div>
    </section>

    {{-- Script menampilkan semua menu --}}
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            const loadMoreButton = document.getElementById("load-more");
            const menuContainer = document.getElementById("menu-container");
            const allMenus = @json($menu); // Data menu dari server

            // Debugging: Pastikan data menu muncul di console
            ...
            loadMoreButton.addEventListener("click", function () {
                ...
                allMenus.forEach((item) => {
                    const menuCard = `
                        <div class="bg-white shadow-md rounded-xl p-6 hover:scale-90 hover:transition-all hover:duration-300">
                            <img src="${item.gambar ? '{{ Storage::url('') }}' + item.gambar : '/img/default.jpg'}" alt="Makanan" class="w-full h-60 object-cover rounded-lg mb-4">
                            <h3 class="text-xl font-semibold">${item.nama}</h3>
                            <p class="text-gray-600">${item.deskripsi}</p>
                            <div class="flex justify-between items-center mt-4">
                                <div class="flex justify-start">
                                    <p class="text-gray-700 text-lg font-bold">Rp${new Intl.NumberFormat('id-ID').format(item.harga)}</p>
                                </div>
                                <div class="py-4 flex justify-end">
                                    ${item.wa ? `<a href="https://wa.me/${item.wa}" class="bg-red-600 text-white px-4 py-2 rounded-3xl">Pesan</a>` : ''}
                                </div>
                            </div>
                        </div>
                    `;
                                    ...
                                    <div class="relative p-6">
                                        <figure class="relative z-10">
                                            <blockquote class="p-6 text-lg leading-loose lg:text-xl">
                                                <p class="w-108">{{ $item->review }}</p><br>
